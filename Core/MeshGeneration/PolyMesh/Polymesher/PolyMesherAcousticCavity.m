...
%-------------------------------------------------------------------------%
function [region,P] = PolyMesherAcousticCavity(Domain,NElem,MaxIter,P,Dati)
...
while(It<=MaxIter && Err>Tol)
  ...
  if NElem<=5000, PolyMshr_PlotMsh(Node,Element,NElem); end; 
end
...
id_el = 2*ones(ne,1);
for i = 1 : NElem
    v = mean(coord(Element{i},:));
    dcenter = sqrt((v(1)-Dati.circle.Center(1)).^2 + (v(2)-Dati.circle.Center(2)).^2);
    if dcenter <= Dati.circle.Radius
        id_el(i) = 1;
    end
end
id_el_new = 0*id_el;
...
% Order first id == 1 and the id == 2
k = 1;
for id = 1 : 2
    for i = 1:length(Element)
        if id_el(i) == id
            connectivity{k} = Element{i};
            nedge(k) = length(Element{i});
            coords_element{k} = coord(Element{i},:);
            x_min = min( coords_element{k}(:,1));x_max = max( coords_element{k}(:,1));
            y_min = min( coords_element{k}(:,2));y_max = max( coords_element{k}(:,2));
            BBox(k,:)=[x_min x_max y_min y_max];
            id_el_new(k) = id;

            elem_area(k) = polyarea(coords_element{k}(:,1),coords_element{k}(:,2));
            max_kb{k} = zeros(nedge(k),1);
            for j = 1:nedge(k)
                if j<nedge(k)
                    v1 = coords_element{k}(j,:); v2 = coords_element{k}(j+1,:);
                    ch_j = j;
                    ch_j_1 = j +1;
                else
                    v1 = coords_element{k}(j,:); v2 = coords_element{k}(1,:);
                    ch_j = j;
                    ch_j_1 = j +1;
                end
                for kk = 1:nedge(k)
                    if kk~=ch_j && kk~=ch_j_1
                        v3 = coords_element{k}(kk,:);
                        [x_tria,y_tria]=poly2cw([v1(1) v2(1) v3(1)],[v1(2) v2(2) v3(2)]);
                        [x1,y1] = polybool('intersection',coords_element{k}(end:-1:1,1),coords_element{k}(end:-1:1,2),x_tria,y_tria);
...
region.connectivity = connectivity; %Element;
