...
%> to generate a cartesian grid of a four-way partitioned rectangular domain.
%>
...
function [region] = PolyMesher_QuadriDomainCartesian(Domain,NElem,MaxIter,P)
...
% [P1] = select_point(Pc);
% [P2] = reflect_pointx(P1);
% [P] = reflect_pointy(P2);

while(It<=MaxIter && Err>Tol)
    Alpha = c*sqrt(Area/NElem);
    %     P = [Pc(1:NElem/2,:);[Pc(1:NElem/2,1),-Pc(1:NElem/2,2)]];   %Lloyd's update
    %     P = [Pc(1:NElem/4,:);...
    %         [-Pc(1:NElem/4,1),Pc(1:NElem/4,2)];...
    %         [-Pc(1:NElem/4,1),-Pc(1:NElem/4,2)];...
    %         [ Pc(1:NElem/4,1),-Pc(1:NElem/4,2)]];   %Lloyd's update
    P1  = Pc.*(Pc(:,1)>0).*(Pc(:,2)>0);
    i=1;
    for k = 1 : NElem
        if P1(k,1)~=0
    ...
    P = [P2; [-P2(:,1) P2(:,2)]; [-P2(:,1) -P2(:,2)];[P2(:,1) -P2(:,2)]];
    %Lloyd's update
    %     [P1] = select_point(Pc);
    %     [P2] = reflect_pointx(P1);
    %     [P] = reflect_pointy(P2);
    ...
    [Pc,A,id_el] = PolyMshr_CntrdPly(Element,Node,NElem);
    Area = sum(abs(A));
...
% [row_coord,col_coord] = size(coord);
% RotMat = [0 -1; 1 0];
%
%
% for i = 1:row_coord
%
%     %temp=coord(i,1);
%     temp = RotMat*[coord(i,1);coord(i,2)];
%     coord(i,1)=temp(1);
%     coord(i,2)=temp(2);
%
% end
%
% [row_BBox,col_BBox] = size(BBox);
%
% for i = 1:row_BBox
%
%     temp=[BBox(i,1) BBox(i,2)];
%     BBox(i,1) = -BBox(i,4);
%     BBox(i,2) = -BBox(i,3);
%     BBox(i,3) = temp(1);
%     BBox(i,4) = temp(2);
...
% for k=1:NElem
%
%     [row,col] = size(coords_element{k});
%     for i=1:row
%         %temp=coords_element{k}(i,1);
%         %RotMat = [0 -1; 1 0];
%         temp = RotMat*[coords_element{k}(i,1);coords_element{k}(i,2)];
%         coords_element{k}(i,1)=temp(1);
%         coords_element{k}(i,2)=temp(2);
%         %plot(coords_element{k}(i,1),coords_element{k}(i,2),'*');
%         %hold on;
%     end
% end
...
function [Q] = reflect_pointx(P)
Q = [P(:,:); [-P(:,1), P(:,2)]];



function [Q] = reflect_pointy(P)
Q = [P(:,:); [P(:,1), -P(:,2)]];
...
function [Pc,A,id] = PolyMshr_CntrdPly(Element,Node,NElem)
...
for el = 1:NElem
    ...
    %%% FLAG CONTROLLO REGIONE
    if Pc(el,1) < -1e-5 && Pc(el,2) < -1e-5
        id(el) = 1;
    elseif Pc(el,1) < -1e-5 && Pc(el,2) > 1e-5
        id(el) = 2;
    elseif Pc(el,1) > 1e-5 && Pc(el,2) > 1e-5
        id(el) = 3;
    else
