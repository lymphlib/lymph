...
%> @brief The function evaluates Legendre polynomials and their first and
%> second derivatives
%> 
%> The function evaluates the Legendre polynomials, its gradient and laplacian. 
...
%> \f[ P''_{i+1}(x) = x P''_{i}(x) + (i+1)P'_{i}(x) \f]
%>
%======================================================================
%> @section classLapLegendreP Class description
%======================================================================
%> @brief The function evaluates Legendre polynomials and their first and
%> second derivatives
%>
...
%> @retval d2P  Matrix containing at the position \f$P_{ij}''\f$ a second derivative of a Legendre polynomial of order \f$N_j\f$ evaluated at the point \f$x_i\f$.
%======================================================================

function [P, dP, d2P] = LapLegendreP(x,N)
    ...
    d2Ptable = zeros(length(x), Nmax + 1);
    
    Ptable(:, 2)   = x;
    dPtable(:, 2)  = ones(length(x), 1);
    d2Ptable(:, 2) = zeros(length(x), 1);
    ...
    for i = 1:Nmax-1
        ...
        d2Ptable(:, i+2) = (i + 2) * dPtable(:, i+1) + x .* d2Ptable(:, i+1);
    end
