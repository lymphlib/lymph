...
%> @author Mattia Corti
%> @date 3 October 2023
%> @brief Assembly of the matrices for the heat equation (quadrature-free)
%
%==========================================================================
%> @section classMatrixHeat Class description
%==========================================================================
%> @brief            Assembly of the mass and stiffness matrices for the heat equation (quadrature-free).
%
...
function [Matrices] = MatrixHeatQF(Data, neighbor, femregion)
    ...
    M_prj_loc = zeros(femregion.nbases^2,femregion.nel);      % Mass Matrix
    
    M_loc = zeros(femregion.nbases^2,femregion.nel);      % Mass Matrix
    ...
    for ie = 1: femregion.nel 
        ...
        mu    = Data.mu{1}(0,0);
	    sigma = Data.sigma{1}(0,0);

        % Mass matrix assembly
        M_prj_loc(:,ie)  = Integral.phiphiC;

        % Stiffness matrix assembly
        M_loc(:,ie) = sigma*Integral.phiphiC;
    ...
    M_prj = sparse(ii_index_QF,jj_index_QF,M_prj_loc,femregion.ndof,femregion.ndof);
    M    = sparse(ii_index_QF,jj_index_QF,M_loc,femregion.ndof,femregion.ndof);
