%> @file   FacesMatricesAssemblyElastodynamics.m
...
%>
%==========================================================================
%> @section classFacesMatricesAssemblyElastodynamics Class description
%==========================================================================
%> @brief            Assembly of the matrices associated with face's 
%> integrals for elastodynamics problem.
%>
...
function [Matrices] = FacesMatricesAssemblyElastodynamics(Data, femregion, Matrices, face, AssembInfo)
    ...
    vs    = Data.vs_el{femregion.id(ie)}(face.xq,face.yq);
    vp    = Data.vp_el{femregion.id(ie)}(face.xq,face.yq);
    mu    = Data.mu_el{femregion.id(ie)}(face.xq,face.yq);
    lam   = Data.lam_el{femregion.id(ie)}(face.xq,face.yq);
    
    % Internal face
    if face.neigh_ie > 0
        mu_n   = Data.mu_el{femregion.id(face.neigh_ie)}(face.xq,face.yq);
        lam_n  = Data.lam_el{femregion.id(face.neigh_ie)}(face.xq,face.yq);
    ...
    if face.neigh_ie == -1
        ...
        Matrices.S1_P_loc(bas_ie,bas_ie) = Matrices.S1_P_loc(bas_ie,bas_ie) + face.penalty_geom.harm(iedg) * (face.ds .* harm_ave .* face.phiedgeq)' * face.phiedgeq;
        Matrices.S4_P_loc(bas_ie,bas_ie) = Matrices.S4_P_loc(bas_ie,bas_ie) + face.penalty_geom.harm(iedg) * (face.ds .* harm_ave .* face.phiedgeq)' * face.phiedgeq;
    ...
    elseif face.neigh_ie > 0
        ...
        Matrices.S1_P_loc(face.neigh_idx,bas_neigh_ie) = Matrices.S1_P_loc(face.neigh_idx,bas_neigh_ie) - (face.ds .* (harm_ave .* face.penalty_geom.harm(iedg)) .* face.phiedgeq)' * face.phiedgeqneigh;
        Matrices.S4_P_loc(face.neigh_idx,bas_neigh_ie) = Matrices.S4_P_loc(face.neigh_idx,bas_neigh_ie) - (face.ds .* (harm_ave .* face.penalty_geom.harm(iedg)) .* face.phiedgeq)' * face.phiedgeqneigh;
