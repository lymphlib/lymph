...
function [F] = ForEla(Data, neighbor, femregion)

%% Quadrature values
[ref_qNodes_1D, w_1D, ref_qNodes_2D, w_2D] = Quadrature(femregion.nqn);

%% Initialization of the forcing term
F1 = spalloc(femregion.ndof_e,1,femregion.ndof_e);
F2 = spalloc(femregion.ndof_e,1,femregion.ndof_e);

G1 = spalloc(femregion.ndof_e,1,femregion.ndof_e);
G2 = spalloc(femregion.ndof_e,1,femregion.ndof_e);
