% Domain Acu --> (-1,0)x(0,1) U (0,1)x(0,1) <-- Ela 
Data.name = 'DataTestElaAcu';
Data.NPhys = 2;
...
Data.TagBcAcu   = [3 7 8]; % Boundary tag
...
Data.source_ue   = {@(x,y)  -((16*cos((pi*x)/2).*sin(pi*y) -  4*x.^2*pi^2.*cos((pi*x)/2).*sin(pi*y) - 3*x.^2*pi^2.*cos(pi*y).*sin((pi*x)/2) + 12*x*pi.*cos((pi*x)/2).*cos(pi*y) - 16*x*pi.*sin((pi*x)/2).*sin(pi*y) + 4*x.^2*pi^2.*cos((pi*x)/2).*sin(pi*y)))/2; ...
                    @(x,y) -(( 8*cos((pi*x)/2).*sin(pi*y) - 17*x.^2*pi^2.*cos((pi*x)/2).*sin(pi*y) - 6*x.^2*pi^2.*cos(pi*y).*sin((pi*x)/2) + 24*x*pi.*cos((pi*x)/2).*cos(pi*y) -  8*x*pi.*sin((pi*x)/2).*sin(pi*y) + 8*x.^2*pi^2.*cos((pi*x)/2).*sin(pi*y)))/4};
Data.source_ue_t = {@(t) cos(pi*sqrt(2)*t)};
...
% Dirichlet BC
Data.DirBCEla  = {@(x,y) x.^2.*cos(pi/2*x).*sin(pi*y); ...
                  @(x,y) x.^2.*cos(pi/2*x).*sin(pi*y)};

Data.DirBCEla_t = {@(t) cos(sqrt(2)*pi*t)};

% exact solution --> used to compute the initial conditions and Dirichlet
% (time dependence)
Data.ue_ex    =  {@(x,y) x.^2.*cos(pi/2*x).*sin(pi*y); ...
                  @(x,y) x.^2.*cos(pi/2*x).*sin(pi*y)};
Data.ue_t_ex  =  {@(t) cos(sqrt(2)*pi*t)};
Data.due_t_ex =  {@(t) -sqrt(2)*pi*sin(sqrt(2)*pi*t)};

% exact gradient --> used for the error analysis
% du1/dx, du1/dy, du2/dx, du2/dy
Data.grad_ue_ex =  {@(x,y) 2.*x.*cos(pi/2*x).*sin(pi*y)-1/2*pi*x.^2.*sin(pi/2*x).*sin(pi*y); ...
                    @(x,y) pi*x.^2*cos(pi/2*x).*cos(pi*y); ...
                    @(x,y) 2.*x.*cos(pi/2*x).*sin(pi*y)-1/2*pi*x.^2.*sin(pi/2*x).*sin(pi*y); ...
                    @(x,y) pi*x.^2*cos(pi/2*x).*cos(pi*y)};
