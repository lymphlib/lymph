...
function [Matrices] = MatPoroQF(Data, neighbor, femregion)
...
ii_index_QF = zeros(femregion.nbases^2,femregion.nel_p);
jj_index_QF = zeros(femregion.nbases^2,femregion.nel_p);
...
El.V4_loc = zeros(femregion.nbases^2,femregion.nel_p);

El.M1_P_rhof_loc = zeros(femregion.nbases^2,femregion.nel_p);
El.M1_P_rhow_loc = zeros(femregion.nbases^2,femregion.nel_p);
El.M1_P_rho_loc  = zeros(femregion.nbases^2,femregion.nel_p);

El.M1_P_eta_kper_loc  = zeros(femregion.nbases^2,femregion.nel_p);
El.M1_P_rho2_zeta_loc = zeros(femregion.nbases^2,femregion.nel_p);
El.M1_P_rho_zeta2_loc = zeros(femregion.nbases^2,femregion.nel_p);

El.MPrjP_1_loc   = zeros(femregion.nbases^2,femregion.nel_p);
                  
El.B1_loc  = zeros(femregion.nbases^2,femregion.nel_p);
El.B2_loc  = zeros(femregion.nbases^2,femregion.nel_p);
El.B3_loc  = zeros(femregion.nbases^2,femregion.nel_p);
El.B4_loc  = zeros(femregion.nbases^2,femregion.nel_p);

El.B1_beta_loc  = zeros(femregion.nbases^2,femregion.nel_p);
El.B2_beta_loc  = zeros(femregion.nbases^2,femregion.nel_p);
El.B3_beta_loc  = zeros(femregion.nbases^2,femregion.nel_p);
El.B4_beta_loc  = zeros(femregion.nbases^2,femregion.nel_p);

El.B1_beta2_loc  = zeros(femregion.nbases^2,femregion.nel_p);
El.B2_beta2_loc  = zeros(femregion.nbases^2,femregion.nel_p);
El.B3_beta2_loc  = zeros(femregion.nbases^2,femregion.nel_p);
El.B4_beta2_loc  = zeros(femregion.nbases^2,femregion.nel_p);

El.P1_loc  = zeros(femregion.nbases^2,femregion.nel_p);
El.P2_loc  = zeros(femregion.nbases^2,femregion.nel_p);

El.P1_beta_loc  = zeros(femregion.nbases^2,femregion.nel_p);
...
for ie = 1 : femregion.nel
    ...
    if tag_ie == 'P'
        ...
        par.mu        = Data.mu{id_ie}(0,0);
        par.lam       = Data.lam{id_ie}(0,0);
        par.beta      = Data.beta{id_ie}(0,0);
        par.m         = Data.m{id_ie}(0,0);
        par.rho_f     = Data.rho_f{id_ie}(0,0);
        par.rho_w     = Data.rho_w{id_ie}(0,0);
        par.rho       = Data.rho{id_ie}(0,0);
        par.eta_kper  = Data.eta{id_ie}(0,0)./Data.k_per{id_ie}(0,0);
        par.rho2_zeta = 2 * Data.rho{id_ie}(0,0) .* Data.zetap{id_ie}(0,0);
        par.rho_zeta2 = Data.rho{id_ie}(0,0) .* Data.zetap{id_ie}(0,0).^2;


        %% Local matrix assembling

        El.V1_loc(:,ie_sh) = (par.lam+2*par.mu)*Integral.gradxgradxC + par.mu*Integral.gradygradyC;
        El.V2_loc(:,ie_sh) = par.lam*Integral.gradygradxC + par.mu*Integral.gradxgradyC;
        El.V3_loc(:,ie_sh) = par.lam*Integral.gradxgradyC + par.mu*Integral.gradygradxC;
        El.V4_loc(:,ie_sh) = (par.lam+2*par.mu)*Integral.gradygradyC + par.mu*Integral.gradxgradxC;

        El.M1_P_rhof_loc(:,ie_sh) = par.rho_f*Integral.phiphiC;
        El.M1_P_rhow_loc(:,ie_sh) = par.rho_w*Integral.phiphiC;
        El.M1_P_rho_loc(:,ie_sh)  = par.rho*Integral.phiphiC;
        
        El.MPrjP_1_loc(:,ie_sh)  = Integral.phiphiC;

        El.M1_P_rho2_zeta_loc(:,ie_sh) = par.rho2_zeta*Integral.phiphiC;
        El.M1_P_rho_zeta2_loc(:,ie_sh) = par.rho_zeta2*Integral.phiphiC;
        El.M1_P_eta_kper_loc(:,ie_sh)  = par.eta_kper*Integral.phiphiC;

        El.B1_loc(:,ie_sh) = par.m*Integral.gradxgradxC;
        El.B2_loc(:,ie_sh) = par.m*Integral.gradygradxC;
        El.B3_loc(:,ie_sh) = par.m*Integral.gradxgradyC;
        El.B4_loc(:,ie_sh) = par.m*Integral.gradygradyC;

        El.B1_beta_loc(:,ie_sh) = (par.m .* par.beta)*Integral.gradxgradxC;
        El.B2_beta_loc(:,ie_sh) = (par.m .* par.beta)*Integral.gradygradxC;
        El.B3_beta_loc(:,ie_sh) = (par.m .* par.beta)*Integral.gradxgradyC;
        El.B4_beta_loc(:,ie_sh) = (par.m .* par.beta)*Integral.gradygradyC;

        El.B1_beta2_loc(:,ie_sh) = (par.m .* par.beta.^2)*Integral.gradxgradxC;
        El.B2_beta2_loc(:,ie_sh) = (par.m .* par.beta.^2)*Integral.gradygradxC;
        El.B3_beta2_loc(:,ie_sh) = (par.m .* par.beta.^2)*Integral.gradxgradyC;
        El.B4_beta2_loc(:,ie_sh) = (par.m .* par.beta.^2)*Integral.gradygradyC;

        El.P1_loc(:,ie_sh) = - par.m*Integral.gradxphiC;
        El.P2_loc(:,ie_sh) = - par.m*Integral.gradyphiC;

        El.P1_beta_loc(:,ie_sh) = - (par.m .* par.beta)*Integral.gradxphiC;
        El.P2_beta_loc(:,ie_sh) = - (par.m .* par.beta)*Integral.gradyphiC;
...
A.V1 = sparse(ii_index_QF,jj_index_QF,El.V1_loc,femregion.ndof_p,femregion.ndof_p);
A.V2 = sparse(ii_index_QF,jj_index_QF,El.V2_loc,femregion.ndof_p,femregion.ndof_p);
A.V3 = sparse(ii_index_QF,jj_index_QF,El.V3_loc,femregion.ndof_p,femregion.ndof_p);
A.V4 = sparse(ii_index_QF,jj_index_QF,El.V4_loc,femregion.ndof_p,femregion.ndof_p);

A.M1_P_rhof  = sparse(ii_index_QF,jj_index_QF,El.M1_P_rhof_loc,femregion.ndof_p,femregion.ndof_p);
A.M1_P_rhow  = sparse(ii_index_QF,jj_index_QF,El.M1_P_rhow_loc,femregion.ndof_p,femregion.ndof_p);
A.M1_P_rho  = sparse(ii_index_QF,jj_index_QF,El.M1_P_rho_loc,femregion.ndof_p,femregion.ndof_p);

A.M1_P_eta_kper  = sparse(ii_index_QF,jj_index_QF,El.M1_P_eta_kper_loc,femregion.ndof_p,femregion.ndof_p);
A.M1_P_rho2_zeta = sparse(ii_index_QF,jj_index_QF,El.M1_P_rho2_zeta_loc,femregion.ndof_p,femregion.ndof_p);
A.M1_P_rho_zeta2 = sparse(ii_index_QF,jj_index_QF,El.M1_P_rho_zeta2_loc,femregion.ndof_p,femregion.ndof_p);

A.MPrjP_1  = sparse(ii_index_QF,jj_index_QF,El.MPrjP_1_loc,femregion.ndof_p,femregion.ndof_p);

A.B1 = sparse(ii_index_QF,jj_index_QF,El.B1_loc,femregion.ndof_p,femregion.ndof_p);
A.B2 = sparse(ii_index_QF,jj_index_QF,El.B2_loc,femregion.ndof_p,femregion.ndof_p);
A.B3 = sparse(ii_index_QF,jj_index_QF,El.B3_loc,femregion.ndof_p,femregion.ndof_p);
A.B4 = sparse(ii_index_QF,jj_index_QF,El.B4_loc,femregion.ndof_p,femregion.ndof_p);

A.B1_beta = sparse(ii_index_QF,jj_index_QF,El.B1_beta_loc,femregion.ndof_p,femregion.ndof_p);
A.B2_beta = sparse(ii_index_QF,jj_index_QF,El.B2_beta_loc,femregion.ndof_p,femregion.ndof_p);
A.B3_beta = sparse(ii_index_QF,jj_index_QF,El.B3_beta_loc,femregion.ndof_p,femregion.ndof_p);
A.B4_beta = sparse(ii_index_QF,jj_index_QF,El.B4_beta_loc,femregion.ndof_p,femregion.ndof_p);

A.B1_beta2 = sparse(ii_index_QF,jj_index_QF,El.B1_beta2_loc,femregion.ndof_p,femregion.ndof_p);
A.B2_beta2 = sparse(ii_index_QF,jj_index_QF,El.B2_beta2_loc,femregion.ndof_p,femregion.ndof_p);
A.B3_beta2 = sparse(ii_index_QF,jj_index_QF,El.B3_beta2_loc,femregion.ndof_p,femregion.ndof_p);
A.B4_beta2 = sparse(ii_index_QF,jj_index_QF,El.B4_beta2_loc,femregion.ndof_p,femregion.ndof_p);

A.P1 = sparse(ii_index_QF,jj_index_QF,El.P1_loc,femregion.ndof_p,femregion.ndof_p);
A.P2 = sparse(ii_index_QF,jj_index_QF,El.P2_loc,femregion.ndof_p,femregion.ndof_p);

A.P1_beta = sparse(ii_index_QF,jj_index_QF,El.P1_beta_loc,femregion.ndof_p,femregion.ndof_p);
A.P2_beta = sparse(ii_index_QF,jj_index_QF,El.P2_beta_loc,femregion.ndof_p,femregion.ndof_p);
